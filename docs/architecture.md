...
3. **The append-only ledgers are guarded by a trigger, not application discipline.** `ai_usage` and `entitlements` reject `UPDATE`/`DELETE`/`TRUNCATE` with Postgres error `42501` unless the transaction first runs `SET LOCAL recharacter.allow_ledger_delete = 'on'` — only account deletion does that. (Defence in depth against app bugs: the owning role could disable the trigger; it is not a boundary against the app itself.)
4. **Owner/case drift is unrepresentable.** `cases(id, owner_id)` is unique and every case-scoped table carries a composite foreign key `(case_id, owner_id) → cases(id, owner_id)` alongside the plain `case_id` one, so a child row can never name a case that belongs to someone else. Every upsert also returns the written row and throws if nothing changed — a `setWhere` miss can never look like a successful save.
5. **AI spend limits count attempts, not completions.** `ai_attempts` is written *before* the per-minute check and the provider call, so concurrent bursts and provider-failure loops are bounded; daily token caps (per owner and the shared `AI_GLOBAL_DAILY_TOKEN_CAP`) are SQL aggregates over `ai_usage` (partial index on managed-tier rows).
6. **Object keys carry the same prefix rule.** Every R2/S3 key is `{ownerId}/{caseId}/{uuid}-{name}`; every read, list, and delete asserts the caller's prefix before touching the key (`assertOwnedKey` in `web/src/lib/case-documents.ts`).
