...
- [x] Adversarial reviews done 2026-08-27/28: two-stage (spec, then adversarial) per task, plus a whole-branch pass. Notable catches: `Referrer-Policy: no-referrer` broke non-JS server-action forms (fixed → `same-origin`); `getCoaching`/`verifySession` were public RPCs (un-exported / moved to `lib/billing-verify.ts`); ID token was reachable via `/api/auth/session` (removed from the session); the deploy job never synced `deploy/docker-compose.yml` to the box (scp step added) and needed a post-deploy page smoke; `APP_BASE_URL` had a localhost default (now required at use).
