...
beforeAll(async () => {
  ...
  alice = await makeUser(`ev_alice_${suffix}@example.test`)
  bob = await makeUser(`ev_bob_${suffix}@example.test`)

  // Alice needs a case row before she can attach case_context / evidence_items to it.
...
test('a user can insert and read their own case_context row; another user sees none', async () => {
  const { data, error } = await alice.client.from('case_context').insert({
    case_id: aliceCaseId,
    owner_id: alice.id,
  ...
  const { data: bobSees } = await bob.client.from('case_context').select('*')
  expect(bobSees).toEqual([]) // RLS filters Alice's row out entirely for Bob
})

test('a user CANNOT spoof-insert a case_context row owned by someone else', async () => {
  const { error } = await bob.client.from('case_context').insert({
    case_id: aliceCaseId,
    owner_id: alice.id,
    condition_category: 'ptsd',
  })
...
test('evidence_items rows are isolated per owner and cannot be updated cross-owner', async () => {
  const { data, error } = await alice.client.from('evidence_items').insert({
    case_id: aliceCaseId,
    owner_id: alice.id,
    item_type: 'nexus_letter',
  }).select().single()
  expect(error).toBeNull()
  expect(data!.owner_id).toBe(alice.id)

  const { data: bobSees } = await bob.client.from('evidence_items').select('*')
...
test('a duplicate (case_id, item_type) insert violates the unique constraint', async () => {
  const { error } = await alice.client.from('evidence_items').insert({
    case_id: aliceCaseId,
    owner_id: alice.id,
    item_type: 'nexus_letter',
  })
